...
let refundReasonSupported = null;

function detectRefundReasonColumn(callback) {
  if (refundReasonSupported !== null) {
    return callback(null, refundReasonSupported);
  }

  db.query("SHOW COLUMNS FROM refunds LIKE 'refund_reason'", function (err, rows) {
    if (err) {
      return callback(err);
    }
    refundReasonSupported = Array.isArray(rows) && rows.length > 0;
    return callback(null, refundReasonSupported);
  });
}
...
function create(orderId, data, callback) {
  ...
  detectRefundReasonColumn(function (detectErr, supportsReason) {
    if (detectErr) {
      if (callback) {
